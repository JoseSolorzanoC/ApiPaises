...
public interface ProvinceRepository extends JpaRepository<Province, Integer> {
        ...
        // Select One Province
        @Query(value="select * from tbprovinces where provinceid=:pid", nativeQuery = true)
        public List<Province> selectOneProvince(
                @Param("pid") int provinceId
        );


         // Get iso2 by id Country
         @Query(value="select iso2_country from tbcountries where countryid=:cid", nativeQuery = true)
